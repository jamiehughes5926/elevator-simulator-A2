...
// Define Problem Size
// --------------------------------------------------
#define NLIFTS 5          // The number of lifts in the building
...
#if defined(SLOW)
    #define LIFTSPEED  10    // The time it takes for the lift to move one floor
...
typedef struct {
    ...
    semaphore floor_mutex;   // Mutex for floor access
} floor_info;
...
typedef struct {
    ...
    semaphore lift_mutex;          // Mutex for lift access
} lift_info;
...
semaphore pick_ex[NLIFTS]; // Mutual exclusion semaphore to prevent people choosing a lift at the same time
...
semaphore lift_lock; // Global lift lock

semaphore lift_mutex_gl; // Global lift mutex
semaphore print_mutex; // Global print mutex
semaphore floor_mutex[NFLOORS]; // Global floor mutexes

// --------------------------------------------------
// Print a string on the screen at position (x,y)
// --------------------------------------------------
void print_at_xy(int x, int y, const char *s) {
    semaphore_wait(&print_mutex);    // Wait for the print_Ex
    ...
    gotoxy(42, NFLOORS+2);
    semaphore_signal(&print_mutex);    // Release the print_ex
}

// --------------------------------------------------
// Function for a lift to pick people waiting on a floor going in a certain direction
// --------------------------------------------------
void get_into_lift(lift_info *lift, int direction) {
    ...
    semaphore_wait(&lift_lock); // Acquire global lift lock
    semaphore_wait(&lift->lift_mutex); // Acquire lift mutex
    ...
    // Check if there are people waiting and the lift isn't full
    while (*waiting && lift->peopleinlift < MAXNOINLIFT) {
        ...
        // Wait for people to get into the lift
        semaphore_wait(&entered[lift->no]);

        // Allow another person to pick a lift
        semaphore_signal(&pick_ex[lift->no]);
    }

    // Release the floor semaphore
    semaphore_signal(&floor_mutex[lift->position]);

    semaphore_signal(&lift->lift_mutex); // Release lift mutex
    semaphore_signal(&lift_lock); // Release global lift lock
}
...
void* lift_thread(void *p) {
    ...
    semaphore_create(&lift.lift_mutex, 1); // Initialize lift mutex
...
void printbuilding(void) {
    ...
    printf( " Submitted by: Jamie Hughes, 21018715\n" );
    printf( "----------------------------------------" );
}
...
int main() {
    ...
    for(i = 0; i < NFLOORS; i++) {
        ...
        semaphore_create(&floors[i].floor_mutex, 1);
    }
    ...
    semaphore_create(&print_ex,1); 
    semaphore_create(&lift_lock,1); //create semaphore with the value 1	
    semaphore_create(&lift_mutex_gl, 1); //create global lift mutex
    semaphore_create(&print_mutex, 1); //create global print mutex
    for(int i = 0; i < NFLOORS; i++) {
        semaphore_create(&floor_mutex[i], 1); //create global floor mutex
    }
